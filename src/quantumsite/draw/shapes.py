...
class Canvas:
    ...
    def __init__(
        ...
        self.height_lims = np.min(coordinates, axis=0)[2], np.max(coordinates, axis=0)[2]
...
class Box:
    ...
    def __init__(self, idx: int, x: float, y: float, z: float, size: float = 5):
        self.idx: int = idx
        self.x: float = x
        self.y: float = y
        self.z: float = z
    ...
    def draw_box(self, figure: go.Figure, height_lims: None | tuple[float, float] = None):
